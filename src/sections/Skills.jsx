  ...
  return (
    ...
    >
      <img
        ...
        width={80}
        height={80}
        style={{ objectFit: 'contain', display: 'block', borderRadius: '18%' }}
      />
    </div>
  )
}
...
function MarqueeRow({ skills, direction }) {
  const anim = direction === 'right'
    ? 'tickerRight 30s linear infinite'
    : 'tickerLeft 30s linear infinite'

  return (
    <div style={{ overflow: 'hidden', width: '100%', position: 'relative' }}>
      {/* Left fade mask */}
      <div aria-hidden="true" style={{
        position: 'absolute', top: 0, left: 0, width: 80, height: '100%', zIndex: 2,
        background: `linear-gradient(to right, ${SECTION_BG}, transparent)`,
        pointerEvents: 'none',
      }} />
      {/* Right fade mask */}
      <div aria-hidden="true" style={{
        position: 'absolute', top: 0, right: 0, width: 80, height: '100%', zIndex: 2,
        background: `linear-gradient(to left, ${SECTION_BG}, transparent)`,
        pointerEvents: 'none',
      }} />

      <div className="skills-track" style={{
        display: 'flex',
        gap: 28,
        width: 'max-content',
        alignItems: 'center',
        ...
        {skills.map((s) => (
          <SkillIcon key={s.name} name={s.name} logo={s.logo} />
        ))}
        {skills.map((s) => (
          <SkillIcon key={`dup-${s.name}`} name={s.name} logo={s.logo} aria-hidden="true" />
        ))}
      </div>
    </div>
  )
